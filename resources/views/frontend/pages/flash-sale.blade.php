                    ...
                    @foreach($flashSaleItems as $flashSaleItem)
                        <div class="col-xl-3 col-sm-6 col-lg-4">
                            <div class="wsus__product_item">
                                ...
                                @if(chackDiscount($flashSaleItem->product))
                                    <span class="wsus__minus">
                                -{{calculateDiscountPercent($flashSaleItem->product->price,$flashSaleItem->product->offer_price)}}%</span>
                                @endif
                                <a class="wsus__pro_link" href="{{route('product-detail',$flashSaleItem->product->slug)}}">
                                    ...
                                    @if(isset($flashSaleItem->product->imagesGallery[0]->image))
                                        <img src="{{asset($flashSaleItem->product->imagesGallery[0]->image)}}" alt="product" class="img-fluid w-100 img_2" />
                                    @else
                                        <img src="{{asset($flashSaleItem->product->thumb_image)}}" alt="product" class="img-fluid w-100 img_2" />
                                    @endif
                                </a>
                                <ul class="wsus__single_pro_icon">
                                    <li><a href="#" data-bs-toggle="modal" data-bs-target="#exampleModal-{{$flashSaleItem->product_id}}"><i
                                ...
                                <div class="wsus__product_details">
                                    ...
                                    <a class="wsus__pro_name" href="{{route('product-detail',$flashSaleItem->product->slug)}}">{{$flashSaleItem->product->name}}</a>
                                    @if(chackDiscount($flashSaleItem->product))
                                        <p class="wsus__price">{{$settings->currency_icon}}{{$flashSaleItem->product->offer_price}}<del>{{$settings->currency_icon}}{{$flashSaleItem->product->price}}</del></p>
                                    @else
                                        <p class="wsus__price">{{$settings->currency_icon}}{{$flashSaleItem->product->price}}</p>
                                    @endif
